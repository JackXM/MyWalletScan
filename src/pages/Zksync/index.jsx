...
function Zksync() {
  ...
  const isRowSatisfyCondition = (record) => {
    ...
    return Object.keys(conditionKeyMapping).every((conditionKey) => {
      if (
        !(conditionKey in zkSyncConfigStore) ||
        zkSyncConfigStore[conditionKey] === null ||
        zkSyncConfigStore[conditionKey] === undefined
      ) {
        return true;
      }
      ...
      <Content>
        ...
        >
          <Form form={walletForm} layout="vertical">
            <Card
              title="设置您的钱包预期标准，如果钱包达到您设置的标准，钱包前面会有绿色提示点"
              bordered={true}
              style={{ width: '100%' }}
            >
              <Row gutter={[16, 16]}>
                <Col span={12}>
                  <FormItem name="ETHTx" addonBefore="ETH Tx数量 ≥ " addonAfter="个" />
                  <FormItem
                    name="zkSyncLiteMinTx"
                    addonBefore="zkSyncLite Tx数量 ≥ "
                    addonAfter="个"
                  />
                  <FormItem
                    name="zkSyncEraMinTx"
                    addonBefore="zkSyncEra Tx数量 ≥ "
                    addonAfter="个"
                  />
        ...
        <Spin spinning={tableLoading}>
          <Table
            ...
            summary={(pageData) => {
              ...
              let totalFees = 0;
              pageData.forEach(
                ({ eth_balance, zks1_balance, zks2_balance, zks2_usdcBalance, totalFee }) => {
              ...
              const emptyCells = Array(10)
                .fill()
                .map((_, index) => <Table.Summary.Cell index={index + 6} />);
